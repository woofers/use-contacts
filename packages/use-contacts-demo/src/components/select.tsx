...
export const Select: React.FC<{}> = () => {
  ...
  const handleChange = useCallback(
    (key: keyof Data) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <>
      <Box className="flex flex-col gap-y-[2px] max-w-xs rounded-lg overflow-hidden">
        <Text
          ...
          type="button"
          onClick={getContact}
          className="text-left px-3 py-2 bg-slate-200 text-slate-900 !text-base w-full"
        >
        ...
        {contacts.map((contact, i) => (
          <Text
            ...
            className="text-left px-3 py-2 bg-slate-200 text-slate-900 !text-base w-full flex items-center justify-between"
          >
            <span>
              <span>{contact.name?.[0] ?? 'none'}</span> -{' '}
              <span>{JSON.stringify(contact)}</span>
            </span>{' '}
            <span>X</span>
          </Text>
        ))}
      ...
      <Box className="max-w-xs flex px-3 pt-4">
        <Text
          as="label"
          className="select-none text-left text-slate-900 !text-base w-full"
          htmlFor="multiple"
        >
          Multiple?
        </Text>
      ...
      <Box
        className="flex flex-col gap-y-[2px] max-w-xs rounded-lg overflow-hidden"
      ...
      >
        {fields.map(field => (
          <Text
            key={`field-${field}`}
            as="div"
            className="text-left px-3 py-2 bg-slate-200 text-slate-900 !text-base w-full flex justify-between items-center"
          >
            <label htmlFor={field} className="select-none">
              {field}
            </label>
            <input
              id={field}
